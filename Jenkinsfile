                ...
                script{
                    sshagent(['Test_server-Key']) {
                        withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
                            ...
                            sh "scp -o StrictHostKeyChecking=no docker-script.sh ec2-user@172.31.36.254:/home/ec2-user"
                            sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.36.254 'bash ~/docker-script.sh'"
